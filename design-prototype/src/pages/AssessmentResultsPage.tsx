...
export function AssessmentResultsPage({ onUnlock }: AssessmentResultsPageProps) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between p-6 sm:p-7 overflow-y-auto bg-gradient-to-b from-indigo-50/50 via-white to-slate-50/30">
      <div className="flex flex-col gap-6 my-auto">
        ...
        <div className="flex flex-col items-center text-center pt-2">
          ...
          <div className="flex items-center justify-center gap-1.5 mb-1">
            <h2 className="text-slate-900 font-extrabold text-2xl tracking-tight m-0">Great Job!</h2>
            <SparkleIcon className="size-6 text-amber-500 fill-amber-500" />
          </div>
          <span className="inline-block px-3 py-0.5 rounded-full bg-indigo-100/80 text-indigo-700 font-bold text-[11px] tracking-wider uppercase mb-2">
            Intermediate Track
          </span>
          <p className="text-slate-500 text-xs leading-relaxed max-w-[270px] m-0">
        ...
        <div className="flex flex-col gap-3">
          <label className="text-slate-900 font-bold text-xs uppercase tracking-wider block px-1">
            Where would you like to start?
          </label>
          {[
            {
              id: 'zero' as const,
              icon: <FlagIcon className="size-5.5 text-[#4F46E5]" />,
              title: 'Start from zero (A1)',
              desc: 'Build your foundation from scratch.',
            },
            {
              id: 'level' as const,
              icon: <RocketIcon className="size-5.5 text-[#4F46E5]" />,
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className={`font-bold text-[14px] m-0 ${active ? 'text-[#4F46E5]' : 'text-slate-900'}`}>
                      {opt.title}
                    </p>
                  </div>
                  <p className={`text-xs m-0 mt-0.5 ${active ? 'text-indigo-500 font-medium' : 'text-slate-400'}`}>
                    {opt.desc}
                  </p>
                </div>
                ...
                {active ? (
                  ...
                  <div className="w-6 h-6 rounded-full ring-1 ring-slate-300 shrink-0" />
                )}
              </button>
            )
          })}
                ...
                <div key={p.id} className="relative w-full">
                  ...
                  >
                    <div className="text-left flex-1 min-w-0 pr-3">
                      ...
                      <p className={`text-xs m-0 mt-0.5 truncate ${active ? 'text-indigo-500 font-medium' : 'text-slate-400'}`}>
                        {p.desc}
                      </p>
                    </div>

                    <div className="text-right shrink-0">
                      <span className={`font-extrabold text-base ${active ? 'text-[#4F46E5]' : 'text-slate-900'}`}>
                        {p.price}
                      </span>
                      <span className={`text-xs font-normal ${active ? 'text-indigo-400' : 'text-slate-400'}`}>
                        {p.period}
                      </span>
                    </div>
                  </button>
