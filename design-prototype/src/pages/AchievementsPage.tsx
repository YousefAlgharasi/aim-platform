...
export function AchievementsPage({ onBack }: AchievementsPageProps) {
  ...
  return (
    <div className="flex-1 h-full min-h-0 flex flex-col justify-between overflow-y-auto bg-[#F8FAFC] dark:bg-slate-900">
      {/* Top App Bar */}
      <div className="sticky top-0 z-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-5 pt-8 pb-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
        ...
        <span className="font-extrabold text-sm text-slate-900 dark:text-white">Achievements</span>
        <button
          type="button"
          onClick={onBack}
      ...
      <div className="flex-1 p-5 pt-4 pb-16 flex flex-col gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 ring-1 ring-slate-200/70 dark:ring-slate-700 rounded-xl p-4 shadow-2xs flex justify-between items-center">
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
              ...
              <span className="font-bold text-slate-900 dark:text-white text-xs">AIM Milestones</span>
            </div>
            <p className="text-slate-400 dark:text-slate-500 text-[11px] m-0">
              {unlockedCount} of {achievements.length} badges unlocked
            </p>
          </div>
          <span className="text-xs font-black text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 px-2.5 py-1 rounded-lg border border-indigo-200/60 dark:border-indigo-800/60">
            Gold League #3
          </span>
        </div>

        {/* Filter Switcher */}
        <div className="bg-slate-200/50 dark:bg-slate-800 p-1 rounded-xl flex gap-1">
          {[
        ...
        <div className="flex flex-col gap-2.5">
          {filtered.map((item) => (
            <div
              key={item.id}
              className={`bg-white dark:bg-slate-800 ring-1 rounded-xl p-3.5 flex items-center gap-3.5 shadow-2xs transition-all ${
                item.unlocked ? 'ring-slate-200/70 dark:ring-slate-700' : 'ring-slate-200/40 dark:ring-slate-800 opacity-75'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-1 mb-0.5">
                  <p className="font-bold text-slate-900 dark:text-white text-xs m-0 truncate">{item.title}</p>
                  {item.unlocked && (
                    <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.2 rounded-md shrink-0">
                      Unlocked
                    </span>
                  )}
                </div>
                <p className="text-slate-500 dark:text-slate-400 text-[11px] m-0 truncate">{item.description}</p>
