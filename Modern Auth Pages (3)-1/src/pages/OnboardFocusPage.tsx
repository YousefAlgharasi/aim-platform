...
const FOCUS_OPTIONS: { id: FocusOption; icon: React.ReactNode; label: string; sub?: string }[] = [
  { id: 'career', icon: <BriefcaseIcon className="size-[22px] text-[#4F46E5]" />, label: 'Career & Work', sub: 'Professional vocabulary & business English' },
  { id: 'exams', icon: <AcademicIcon className="size-[22px] text-[#4F46E5]" />, label: 'Exams & School', sub: 'IELTS, TOEFL, and academic prep' },
  { id: 'speaking', icon: <ChatIcon className="size-[22px] text-[#4F46E5]" />, label: 'Real-life Speaking', sub: 'Fluency in everyday conversations' },
  { id: 'media', icon: <FilmIcon className="size-[22px] text-[#4F46E5]" />, label: 'Media & Culture', sub: 'Movies, podcasts, and casual slang' },
]
...
export function OnboardFocusPage({ onNext, onSkip }: OnboardFocusPageProps) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between bg-gradient-to-b from-indigo-50/40 via-white to-slate-50/50">
      <ProgressDots total={4} current={1} />

      <div className="px-6 pt-6">
        <span className="text-[#4F46E5] font-extrabold text-xs uppercase tracking-wider block mb-1">
          Step 2 of 4
        </span>
        <h1 className="text-slate-900 font-extrabold text-3xl tracking-tight leading-tight mb-2">
