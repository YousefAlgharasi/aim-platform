...
export function VoiceTeacherPage({
  ...
  return (
    <div className="flex-1 h-full min-h-0 flex flex-col justify-between bg-gradient-to-b from-indigo-50/50 via-white to-slate-50/50">
      {/* Header */}
      ...
      <div className="px-6 pt-6 text-center">
        <span className="text-[#4F46E5] font-extrabold text-xs uppercase tracking-wider bg-indigo-50 px-3 py-1 rounded-full">
          Speak this phrase
        </span>
        <h2 className="text-slate-900 font-extrabold text-2xl tracking-tight leading-snug mt-3 mb-2">
          "{targetPhrase}"
        </h2>
      ...
      <div className="my-auto flex flex-col items-center justify-center py-6">
          ...
          {[4, 8, 14, 22, 16, 28, 18, 12, 24, 15, 8, 5].map((h, i) => (
            <div
              ...
              className={`w-1.5 rounded-full transition-all duration-200 ${
                recording ? 'bg-indigo-600 animate-pulse' : 'bg-slate-200'
              }`}
        ...
        <p className={`text-sm font-bold mt-5 mb-0 ${recording ? 'text-rose-500' : 'text-slate-500'}`}>
          {recording ? 'Listening... Tap to stop' : 'Tap mic & speak out loud'}
        </p>

        {recording && (
          <span className="text-[#4F46E5] font-mono text-xs font-bold mt-1 tracking-widest">
      ...
      {recorded && (
        <div className="mx-6 mb-8 bg-white ring-1 ring-emerald-200 rounded-2xl p-4 flex items-center gap-4 shadow-md shadow-emerald-500/10 animate-in fade-in slide-in-from-bottom-2">
          <div className="size-12 rounded-xl bg-emerald-500 text-white flex items-center justify-center shrink-0 font-black text-lg">
            96%
          </div>
          <div>
            <p className="font-extrabold text-slate-900 text-sm m-0">Excellent Pronunciation!</p>
