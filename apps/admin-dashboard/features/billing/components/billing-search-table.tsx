...
import { useBackendSearch } from '../../../core/hooks/use-backend-search';
import {
  AdminButton,
  AdminInput,
  AdminTable,
...
export function BillingSearchTable<T, F extends string = string>({
  ...
  const activeFilterClass = `admin-${title.toLowerCase().replace(/\s+/g, '-')}-view__filter--active`;
          ...
          {filters.map((f) => (
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded-lg transition-colors cursor-pointer ${
                filter === f.key
                  ? `bg-[var(--color-primary-500)] text-white ${activeFilterClass}`
