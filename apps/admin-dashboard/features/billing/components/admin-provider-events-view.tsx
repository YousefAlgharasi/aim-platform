...
import { useAdminFetch } from '../../../core/hooks/use-admin-fetch';
import {
  AdminDateCell,
  AdminIdCell,
  AdminStatusBadge,
  ...
  return (
    <section className="flex flex-col gap-4">
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded-lg transition-colors cursor-pointer ${
              filter === f.key
                ? 'bg-[var(--color-primary-500)] text-white admin-provider-events-view__filter--active'
      ...
      <div className="mt-4 p-4 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border)] text-xs text-[var(--text-secondary)]">
        Read-only — provider event validity is decided by backend signature verification.
      </div>
    </section>
  );
}
