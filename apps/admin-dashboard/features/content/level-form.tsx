...
import type { AdminLevelSummary } from './admin-levels-api';
...
export function LevelForm({ mode, initial, onSubmit, onCancel }: Props) {
  ...
  return (
    <div className="cf-card">
      <div className="cf-header">
        <h2 className="cf-title">{mode === 'create' ? 'New Level' : 'Edit Level'}</h2>
      ...
      <form onSubmit={handleSubmit} className="cf-form">
        <div className="cf-field">
          <label htmlFor="lf-title" className="cf-label">Title <span className="cf-req">*</span></label>
          <input id="lf-title" type="text" value={title} onChange={(e) => setTitle(e.target.value)}
            placeholder="e.g. Beginner" disabled={isPending} maxLength={255}
            className={`cf-input ${fieldErrors.title ? 'cf-input--error' : ''}`} />
          {fieldErrors.title && <span className="cf-error">{fieldErrors.title}</span>}
        </div>

        <div className="cf-field">
          <label htmlFor="lf-code" className="cf-label">Code</label>
          <input id="lf-code" type="text" value={code} onChange={(e) => setCode(e.target.value)}
            placeholder="e.g. A1" disabled={isPending} maxLength={50} className="cf-input" />
          <span className="cf-hint">Optional editorial code (e.g. CEFR level).</span>
        </div>

        <div className="cf-field">
          <label htmlFor="lf-slug" className="cf-label">Slug</label>
          <input id="lf-slug" type="text" value={slug} onChange={(e) => setSlug(e.target.value)}
            placeholder="e.g. beginner" disabled={isPending} maxLength={255}
            className={`cf-input ${fieldErrors.slug ? 'cf-input--error' : ''}`} />
          <span className="cf-hint">Optional. Lowercase, hyphens only. Must be unique within a course.</span>
          {fieldErrors.slug && <span className="cf-error">{fieldErrors.slug}</span>}
        </div>

        <div className="cf-field">
          <label htmlFor="lf-desc" className="cf-label">Description</label>
          <textarea id="lf-desc" value={description} onChange={(e) => setDescription(e.target.value)}
            placeholder="Brief summary of this level." disabled={isPending} rows={3} maxLength={2000}
            className="cf-textarea" />
        </div>

        <div className="cf-actions">
          <button type="submit" disabled={isPending} className="cf-submit">
            {isPending ? 'Saving…' : mode === 'create' ? 'Create Level' : 'Save Changes'}
          </button>
