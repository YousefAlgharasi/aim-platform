...
async function fetchEntity(token: string, id: string) {
  try {
    const envelope = await adminApiClient.get<{ id: string; title: string; status: string }>(
      `/curriculum/chapters/${encodeURIComponent(id)}`,
      (v) => {
...
export default async function ChapterStatusPage({ params }: Props) {
  const { chapterId } = await params;
  const cookieStore = await cookies();
  const token = cookieStore.get(ADMIN_AUTH_TOKEN_COOKIE)?.value.trim() ?? '';

  const entity = await fetchEntity(token, chapterId);
  ...
  async function handleTransition(
    ...
    try {
      if (action === 'publish') await publishContent(t, 'chapters', chapterId);
      else if (action === 'archive') await archiveContent(t, 'chapters', chapterId);
      else await restoreContent(t, 'chapters', chapterId);
      return {};
    } catch (err) {
      const msg =
  ...
  return (
    <section className="admin-curriculum-page">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        ...
        <Link href="/admin/content/chapters">Chapters</Link>
        <span aria-hidden="true">/</span>
        <span>Status</span>
      </nav>
      ...
      {entity ? (
        ...
        <p className="admin-error-banner" role="alert">Chapter not found or backend unavailable.</p>
      )}
    </section>
  );
}
