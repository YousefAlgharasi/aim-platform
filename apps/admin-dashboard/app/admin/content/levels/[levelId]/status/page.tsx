...
async function fetchEntity(token: string, id: string) {
  try {
    const envelope = await adminApiClient.get<{ id: string; title: string; status: string }>(
      `/curriculum/levels/${encodeURIComponent(id)}`,
      (v) => {
...
export default async function LevelStatusPage({ params }: Props) {
  const { levelId } = await params;
  const cookieStore = await cookies();
  const token = cookieStore.get(ADMIN_AUTH_TOKEN_COOKIE)?.value.trim() ?? '';

  const entity = await fetchEntity(token, levelId);
  ...
  async function handleTransition(
    ...
    try {
      if (action === 'publish') await publishContent(t, 'levels', levelId);
      else if (action === 'archive') await archiveContent(t, 'levels', levelId);
      else await restoreContent(t, 'levels', levelId);
      return {};
    } catch (err) {
      const msg =
  ...
  return (
    <section className="admin-curriculum-page">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        ...
        <Link href="/admin/content/levels">Levels</Link>
        <span aria-hidden="true">/</span>
        <span>Status</span>
      </nav>
      ...
      {entity ? (
        ...
        <p className="admin-error-banner" role="alert">Level not found or backend unavailable.</p>
      )}
    </section>
  );
}
