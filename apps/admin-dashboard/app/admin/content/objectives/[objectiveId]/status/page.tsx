...
import {
  ...
  restoreContent,
  type ContentStatus,
} from '../../../../../../features/content/api/admin-content-status-api';
import { ContentStatusWorkflow } from '../../../../../../features/content/components/content-status-workflow';
...
export default async function ObjectiveStatusPage({ params }: Props) {
  const { objectiveId } = await params;
  const cookieStore = await cookies();
  const token = cookieStore.get(ADMIN_AUTH_TOKEN_COOKIE)?.value.trim() ?? '';

  const entity = await fetchEntity(token, objectiveId);
  ...
  async function handleTransition(
    ...
    try {
      if (action === 'publish') await publishContent(t, 'objectives', objectiveId);
      else if (action === 'archive') await archiveContent(t, 'objectives', objectiveId);
      else await restoreContent(t, 'objectives', objectiveId);
      return {};
    } catch (err) {
      const msg =
  ...
  return (
    <section className="admin-curriculum-page">
      <nav className="admin-breadcrumb" aria-label="Breadcrumb">
        ...
        <Link href="/admin/content/objectives">Objectives</Link>
        <span aria-hidden="true">/</span>
        <span>Status</span>
      </nav>
      ...
      {entity ? (
        ...
        <p className="admin-error-banner" role="alert">Objective not found or backend unavailable.</p>
      )}
    </section>
  );
}
