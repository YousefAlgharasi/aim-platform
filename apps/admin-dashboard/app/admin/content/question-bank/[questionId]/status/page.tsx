...
export default async function QuestionStatusPage({ params }: Props) {
  const { questionId } = await params;
  const cookieStore = await cookies();
  const token = cookieStore.get(ADMIN_AUTH_TOKEN_COOKIE)?.value.trim() ?? '';

  const entity = await fetchEntity(token, questionId);
  ...
  return (
    <section className="admin-curriculum-page">
      ...
      {entity ? (
        ...
        <p className="admin-error-banner" role="alert">Question not found or backend unavailable.</p>
      )}
    </section>
  );
}
