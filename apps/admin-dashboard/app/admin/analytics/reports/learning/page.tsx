...
export default async function AdminLearningReportsPage() {
  ...
  try {
    ...
    fetchError = error instanceof AdminApiClientError
      ...
      : 'Failed to load learning report definitions.';
  }

  return (
    <section className="lr-page">
      <nav className="lr-breadcrumb">
        <Link href="/admin/analytics" className="lr-breadcrumb-link">Analytics</Link>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 5l7 7-7 7"/></svg>
        <span className="lr-breadcrumb-current">Learning Reports</span>
      </nav>
      ...
      <style>{`
        ...
        .lr-breadcrumb { display: flex; align-items: center; gap: 6px; font-size: 13px; color: var(--text-muted); }
        .lr-breadcrumb-link { color: var(--text-link); text-decoration: none; }
        .lr-breadcrumb-link:hover { text-decoration: underline; }
        .lr-breadcrumb-current { color: var(--text-secondary); font-weight: 500; }
