# Phase 2 — Task Execution Rules

## Purpose

This document defines the official execution rules for **Phase 2 — Auth, Users, Roles** tasks in the AIM Platform.

It standardizes how Phase 2 tasks are selected, dependency-checked, claimed in Notion, implemented on Git branches, committed, pushed, documented, and completed.

These rules exist to keep Phase 2 consistent with the Phase 0 and Phase 1 execution model while preserving the Phase 2 scope boundary defined in:
...
Phase 2 is limited to authentication, users, profiles, roles, permissions, ownership, backend guards, Flutter auth/profile flow, admin users/roles foundation, and security review.

Phase 2 must not include onboarding, placement, lessons, practice, sessions, AIM integration, dashboard recommendations, review/retention, progress reports, AI Teacher, or Student Web App.
...
An agent may work on only one Phase 2 task per run.
...
6. The dependency output matches the expected path or output type from `docs/tasks/phase_2_task_prompts.md`.
...
git checkout -b phase2/P2-002-phase-2-task-rules
```

If the branch already exists locally:

```bash
git checkout phase2/P2-002-phase-2-task-rules
git pull origin phase2/P2-002-phase-2-task-rules
```

If the branch exists remotely but not locally:

```bash
git fetch origin phase2/P2-002-phase-2-task-rules
git checkout -b phase2/P2-002-phase-2-task-rules origin/phase2/P2-002-phase-2-task-rules
```
...
For Flutter tasks, edit only the relevant Flutter files.

For admin dashboard tasks, edit only the relevant admin files.

Do not modify Phase 0 or Phase 1 decisions unless a documented conflict requires it.
...
- Backend role checks are final.
- Backend permission checks are final.
- Backend ownership checks are final.
- Flutter/Admin role checks are UX only.
- Flutter/Admin must not become the source of truth for permissions.
- Clients must not receive service-role keys or privileged credentials.
- Clients must not call AIM Engine directly.
- Clients must not calculate mastery, level, weakness, difficulty, retention, or recommendations.
- Student Web App work is out of scope.
- Onboarding, placement, lessons, practice, sessions, AIM integration, recommendations, progress reports, and AI Teacher work are out of scope.
...
Do not copy secrets into documentation, comments, examples, Flutter code, admin code, or tests.
...
```bash
git add docs/phase-2/task-execution-rules.md
git commit -m "P2-002: create phase 2 task execution rules"
```
...
```bash
git push origin phase2/P2-002-phase-2-task-rules
```
...
- The exact task section is missing from `docs/tasks/phase_2_task_prompts.md`.
- The required branch is missing or cannot be created.
- The working tree has unrelated changes.
- A Phase 0, Phase 1, and Phase 2 conflict is found.
- A real secret is detected.
- The task requires Student Web App work.
- The task requires onboarding, placement, lessons, practice, sessions, AIM integration, recommendations, progress reports, review/retention, or AI Teacher work.
