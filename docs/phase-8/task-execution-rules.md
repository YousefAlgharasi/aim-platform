...
Phase 8 tasks. Every contributor must follow these rules precisely.
...
- Open the Phase 8 Notion Tasks database.
...
- Open `docs/tasks/phase8_prompts.md`.
...
- Do not implement AI Teacher logic that overrides AIM Engine authority.
...
- Example: `P8-002: create phase 8 task execution rules`
...
- [ ] AI Teacher does not call any AI provider directly from Flutter.
...
- In Notion, insert a completion comment on the task page using the
  ...
  - Commit hash(es).
  - Done Test results.
  - Phase 8 validation results (AI provider access, AIM Engine
...
| The prompt section is missing in `phase8_prompts.md` | Stop. Do not improvise. |
| Secrets or credentials are found in task inputs | Stop. Do not commit. |
| The task would cause Flutter to call an AI provider directly | Stop. Redesign to a backend call. |
...
- Never commit AI provider API keys, Supabase service-role keys,
  database credentials, production tokens, or other secrets.
...
- Do not implement other Phase 8 tasks while working on the current one.
...
- AI Teacher may explain, guide, hint, and tutor using backend-approved
  context, but never recomputes or overrides AIM Engine decisions.
- If a task appears to require AI Teacher to make a learning decision,
  stop and escalate.

### 4.4 AI Provider Access Boundary

- All AI provider access happens through the backend-only AI Provider
  Gateway (Group F).
- Flutter never calls an AI provider directly and never holds an AI
  provider API key.
- The backend gateway is the only place provider keys are read, from
...
- Shared components (buttons, cards, inputs, chat bubbles, loading and
  error states, navigation) must use the existing widget library from
...
- Mirror icons that imply direction (back arrows, navigation chevrons,
  chat bubble tails).
- Verify chat bubble alignment and text flow for Arabic before marking
  a UI task Done.
...
3. Verify → Prompt section #P8-XXX exists
...
- Phase 8 Charter: `docs/phase-8/ai-teacher-text-charter.md`
- Phase 8 Prompts: `docs/tasks/phase8_prompts.md`
- Repository: https://github.com/YousefAlgharasi/aim-platform

---

*Task execution rules created: P8-002 | Branch: phase8/P8-002-phase-8-task-rules*
