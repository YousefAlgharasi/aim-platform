...
- All skill IDs in lessons must exist in `docs/learning/english-skill-tree.md`. Orphaned skill IDs are a validation error at content authoring time. Valid categories are: PHO, VOC, GRA, READ, WRITE, LIS, SPE.
...
| Should a lesson be able to target Listening (LIS) skills in MVP? | LIS skills are valid references in `primary_skill_ids` and `supporting_skill_ids`. However, serving LIS lesson blocks requires confirmed audio delivery infrastructure. Until audio is confirmed, lessons must not set LIS skills as primary targets for live student sessions. |
| Should a lesson be able to target Speaking (SPE) skills in MVP? | SPE skills are valid references in `primary_skill_ids` and `supporting_skill_ids` at the planning level. Automated free-speech scoring is out of MVP scope. SPE lesson blocks must not implement client-side speech scoring. Any SPE use in MVP must be structured (prompted repetition, read-aloud with manual review) and explicitly approved. |
