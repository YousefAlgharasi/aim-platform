...
Writing is excluded from placement because letter formation and copying tasks are not reliably scorable with automated scoring in MVP. The AIM Engine assigns writing skill states after the first lesson.
...
- The placement test is text and image based only in MVP. Audio-based phonics placement is deferred until audio delivery is confirmed.
