...
export class StudentSkillStateReadService {
  ...
  async getSkillStatesForStudent(
    ...
    // No fabricated fallback: when a student has no rows in
    // student_skill_states yet, this returns an honest empty array rather
    // than inventing mastery numbers the student never earned. Callers
    // must treat an empty skillStates array as "no skill state yet".
