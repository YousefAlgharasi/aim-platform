...
// TTS audio for a question_bank `listening_choice` item's listening_script,
// and stream the resulting bytes back to the student. Mirrors
// PlacementQuestionAudioService's pattern for the curriculum question bank.
//
// Security rules:
//   - Backend is the sole authority for question content; no client-supplied
//     text is ever synthesized.
//   - listening_script (never stem/explanation/correct answer data) is the
//     only text synthesized.
...
export interface SessionQuestionAudioResult {
...
@Injectable()
export class SessionQuestionAudioService {
  private readonly logger = new Logger(SessionQuestionAudioService.name);
  ...
  async ensureAudio(
    ...
    studentId: string,
    languageCode: string,
  ): Promise<SessionQuestionAudioResult> {
    const result = await this.db.query<QuestionBankListeningRow>(
      `SELECT id, type, listening_script, status
         FROM question_bank
        WHERE id = $1
        LIMIT 1`,
      [questionId],
    ...
    if (!this.ttsGateway) {
      this.logger.warn(
        `SessionQuestionAudioService.ensureAudio: TTS_GATEWAY not bound, cannot synthesize questionId=${questionId}`,
      );
    ...
    const response = await this.ttsGateway.synthesize({
      ...
      languageCode,
      sessionId: `curriculum-question:${questionId}`,
      studentId,
    });
    ...
    if (outcome.isFallback || !outcome.audioRef) {
      this.logger.warn(
        `SessionQuestionAudioService.ensureAudio: synthesis failed for questionId=${questionId}`,
      );
      return { audioRef: null, scriptMissing: false };
    }
