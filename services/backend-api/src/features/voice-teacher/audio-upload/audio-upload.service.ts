 ...
 * (steps 3–7: field presence, file size, declared MIME type, declared
 * duration), creates a pending voice message record via
 * VoiceMessageRepository, and returns the message ID. Steps 1–2
 * (auth + session ownership) are handled by the API controller/guard
 * layer before this service is called. Steps 6–8 (actual MIME sniffing
 * and actual duration decoding) are handled by the Audio File Validation
 * task (P9-029). The STT Gateway call is wired by a later orchestration
 * task. This service never calls an STT/TTS/AI provider, never holds
 * provider credentials, and never computes any AIM Engine-owned value.
 */
